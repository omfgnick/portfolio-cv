...
void main(){
  ...
  vec3 col=vec3(.012,.035,.032);
  col=mix(col,vec3(.04,.45,.32),smoothstep(.42,.88,a)*.62);
  col=mix(col,vec3(.06,.55,.62),smoothstep(.52,.95,b)*.38);
  col=mix(col,vec3(.30,.22,.62),smoothstep(.58,.92,c)*.26);
  col=mix(col,vec3(.55,.78,.18),smoothstep(.80,.99,a*b*2.1)*.20);
...
const CSS_FALLBACK: React.CSSProperties = {
  background:
    'radial-gradient(60% 50% at 18% 8%, rgba(16,185,129,.30), transparent 65%),' +
    'radial-gradient(50% 45% at 82% 16%, rgba(34,211,238,.18), transparent 65%),' +
    'radial-gradient(55% 50% at 50% 55%, rgba(139,92,246,.13), transparent 65%),' +
    '#06080a',
}
